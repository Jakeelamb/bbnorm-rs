...
OUT="${1:-tmp/real_inactive_read_filter_options_parity}"
mkdir -p "$OUT"
rm -f "$OUT"/*
...
printf 'Running Java BBNorm inactive read-filter parser-option case...\n'
...
printf 'Running Rust bbnorm-rs inactive read-filter parser-option case...\n'
...
printf 'Inactive read-filter parser-option parity passed. Java: %sms, Rust: %sms\n' "$JAVA_MS" "$RUST_MS"
