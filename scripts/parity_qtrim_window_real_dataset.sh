...
OUT="${1:-tmp/real_qtrim_window_parity}"
mkdir -p "$OUT"
rm -f "$OUT"/*

FIXTURE="$OUT/real_qtrim_window_tail.fq"
...
with open(dest, "w") as writer:
    for copy in range(4):
        writer.write(f"@real_qtrim_window_{copy}\n")
...
printf 'Running Java BBNorm qtrim window real-derived keep-all case...\n'
...
printf 'Running Rust bbnorm-rs qtrim window real-derived keep-all case...\n'
...
printf 'qtrim window parity passed. Java: %sms, Rust: %sms\n' "$JAVA_MS" "$RUST_MS"
