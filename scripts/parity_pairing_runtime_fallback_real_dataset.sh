...
OUT="${1:-tmp/real_pairing_runtime_fallback}"
mkdir -p "$OUT"
rm -f "$OUT"/*
...
printf 'Running Java BBNorm explicit-paired local baseline on paired phiX...\n'
...
printf 'Running Rust bbnorm-rs with global pairing flags; expecting explicit-routing fallback notes...\n'
target/debug/bbnorm-rs \
  "${COMMON[@]}" "${PAIRING_FLAGS[@]}" \
...
grep -q 'pairreads=t is a BBTools global pairing behavior control' "$OUT/rust.stderr.log"
grep -q 'flipr2=t is a BBTools global pairing behavior control' "$OUT/rust.stderr.log"
...
printf 'Pairing runtime fallback passed. Rust emits explicit-routing notes and matches local Java baseline. Logs: %s\n' "$OUT"
