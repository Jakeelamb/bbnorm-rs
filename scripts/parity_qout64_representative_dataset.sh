...
OUT="${1:-tmp/representative_qout64_parity}"
mkdir -p "$OUT"
rm -f "$OUT"/*

READS="$OUT/representative_qout64.fq"
cat > "$READS" <<'FASTQ'
@qout64
ACGTNNACGT
+
...
printf 'Running Java BBNorm qout=64 representative case...\n'
java -Xmx1g -cp "$CP" jgi.KmerNormalize \
  "${COMMON[@]}" \
  "out=$OUT/java.keep.fq" \
  >"$OUT/java.stdout.log" 2>"$OUT/java.stderr.log"

printf 'Running Rust bbnorm-rs qout=64 representative case...\n'
...
expected = "BBhr@@hhhh"
if quality != expected:
    raise SystemExit(f"unexpected qout=64 qualities: {quality!r} != {expected!r}")
PY

printf 'qout=64 representative parity passed. Logs and outputs: %s\n' "$OUT"
