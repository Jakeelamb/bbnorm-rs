...
OUT="${1:-tmp/real_remove_if_either_bad_parity}"
mkdir -p "$OUT"
rm -f "$OUT"/*
...
printf 'Running Java BBNorm remove-if-either-bad alias case...\n'
...
printf 'Running Rust bbnorm-rs remove-if-either-bad alias case...\n'
...
printf 'Remove-if-either-bad alias parity passed. Java: %sms, Rust: %sms\n' "$JAVA_MS" "$RUST_MS"
