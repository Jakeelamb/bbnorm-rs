...
OUT="${1:-tmp/real_normalize_parity}"
mkdir -p "$OUT"
rm -f "$OUT"/*
...
printf 'Running Java BBNorm deterministic error-toss normalization on real paired phiX fixture...\n'
...
printf 'Running Rust bbnorm-rs deterministic error-toss normalization on same fixture...\n'
...
printf 'Deterministic error-toss normalization parity passed. Java: %sms, Rust: %sms\n' "$JAVA_MS" "$RUST_MS"
