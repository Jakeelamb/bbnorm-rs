...
OUT="${1:-tmp/real_interleaved_with_in2_parity}"
mkdir -p "$OUT"
rm -f "$OUT"/*
...
printf 'Running Java BBNorm with in2 plus interleaved=t on paired phiX...\n'
...
printf 'Running Rust bbnorm-rs with in2 plus interleaved=t on paired phiX...\n'
...
grep -q 'Set INTERLEAVED to true' "$OUT/java.stderr.log"

printf 'interleaved=t with in2 parity passed. Logs and outputs: %s\n' "$OUT"
