...
OUT="${1:-tmp/real_interleaved_auto_parity}"
mkdir -p "$OUT"
rm -f "$OUT"/*
...
with gzip.open(r1_path, 'rt') as r1, gzip.open(r2_path, 'rt') as r2, open(out_path, 'w') as out:
    ...
    while True:
        ...
        rec1[0] = f'@pair{pair}/1\n'
        rec2[0] = f'@pair{pair}/2\n'
        out.writelines(rec1)
        out.writelines(rec2)
        pair += 1
PYBUILD
...
printf 'Running Java BBNorm auto-interleaved normalization on paired-name phiX fixture...\n'
...
printf 'Running Rust bbnorm-rs auto-interleaved normalization on same fixture...\n'
...
printf 'Auto-interleaved normalization parity passed. Java: %sms, Rust: %sms\n' "$JAVA_MS" "$RUST_MS"
