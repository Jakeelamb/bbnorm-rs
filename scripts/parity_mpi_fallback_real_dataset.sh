...
printf 'Running Rust bbnorm-rs with enabled MPI flags; expecting local fallback notes...\n'
...
grep -q 'usempi=t is a BBTools MPI execution control' "$OUT/rust.stderr.log"
grep -q 'mpi=2 is a BBTools MPI execution control' "$OUT/rust.stderr.log"
grep -q 'crismpi=t is a BBTools MPI stream control' "$OUT/rust.stderr.log"
grep -q 'mpikeepall=t is a BBTools MPI stream control' "$OUT/rust.stderr.log"
...
printf 'MPI fallback parity passed. Rust emits local-run notes and matches local Java baseline. Logs: %s\n' "$OUT"
