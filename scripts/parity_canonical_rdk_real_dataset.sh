...
OUT="${1:-tmp/real_canonical_rdk_parity}"
mkdir -p "$OUT"
rm -f "$OUT"/*
...
run_case() {
  ...
  printf 'Running Java BBNorm canonical/rdk case %s...\n' "$name"
...
PYTIME
  ...
  printf 'Running Rust bbnorm-rs canonical/rdk case %s...\n' "$name"
...
run_case canonical_false "canonical=f"
run_case keep_duplicate_kmers "rdk=f"
run_case combined "canonical=f" "rdk=f"

printf 'Canonical/rdk parity passed. Outputs and logs: %s\n' "$OUT"
