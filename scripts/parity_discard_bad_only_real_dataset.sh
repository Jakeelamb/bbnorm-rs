...
OUT="${1:-tmp/real_discard_bad_only_parity}"
mkdir -p "$OUT"
rm -f "$OUT"/*
...
if chosen is None:
    raise SystemExit(f"no N-free read found in {source}")
...
printf 'Running Java BBNorm discardbadonly high-depth real-derived case...\n'
...
printf 'Running Rust bbnorm-rs discardbadonly high-depth real-derived case...\n'
...
printf 'Discardbadonly parity passed. Java: %sms, Rust: %sms\n' "$JAVA_MS" "$RUST_MS"
