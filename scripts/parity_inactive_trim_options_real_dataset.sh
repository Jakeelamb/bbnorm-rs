...
OUT="${1:-tmp/real_inactive_trim_options_parity}"
mkdir -p "$OUT"
rm -f "$OUT"/*
...
printf 'Running Java BBNorm inactive trim parser-option case...\n'
...
printf 'Running Rust bbnorm-rs inactive trim parser-option case...\n'
...
printf 'Inactive trim parser-option parity passed. Java: %sms, Rust: %sms\n' "$JAVA_MS" "$RUST_MS"
