...
DATA1="${DATA1:-$BIO_ROOT/reads/short_reads/spombe_972_pe_srr17530188/SRR17530188_1.fastq.gz}"
DATA2="${DATA2:-$BIO_ROOT/reads/short_reads/spombe_972_pe_srr17530188/SRR17530188_2.fastq.gz}"
CP="vendor/BBTools-master/current"
OUT="${1:-tmp/biological_errorstate_java_guard}"
READS="${READS:-1000}"
...
run_case() {
  ...
  mkdir -p "$case_out"
  printf '\nRunning biological Java error-state guard for %s on %s and %s...\n' "$mode_name" "$DATA1" "$DATA2"
  ...
  if [[ "$java_status" -eq 0 ]]; then
    echo "Expected vendored Java to terminate in an error state for $mode_name on this dataset, but it succeeded." >&2
    exit 1
  fi

  grep -q 'IndexOutOfBoundsException' "$case_out/java.stderr.log"
  grep -q 'PairStreamer.nextList' "$case_out/java.stderr.log"
  grep -q 'BBNorm terminated in an error state' "$case_out/java.stderr.log"
  test -s "$case_out/java.hist.tsv"
  test -s "$case_out/java.rhist.tsv"
  if [[ -s "$case_out/java.keep1.fq" && -s "$case_out/java.keep2.fq" ]]; then
    :
  else
    find "$case_out" -maxdepth 1 -type f -name 'TEMPFILE_BBNORM_*java.keep1.fq' -size +0c | grep -q .
    find "$case_out" -maxdepth 1 -type f -name 'TEMPFILE_BBNORM_*java.keep2.fq' -size +0c | grep -q .
  fi
  ...
  test -s "$case_out/rust.keep1.fq"
  test -s "$case_out/rust.keep2.fq"
...
printf '\nBiological Java error-state guard summary:\n'
column -t -s $'\t' "$OUT/summary.tsv" || cat "$OUT/summary.tsv"
printf '\nBiological Java error-state guard passed. Vendored Java emits partial outputs and then terminates in an error state on this paired dataset while Rust completes. Outputs and logs: %s\n' "$OUT"
