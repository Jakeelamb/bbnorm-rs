...
OUT="${1:-tmp/real_minlen_parity}"
mkdir -p "$OUT"
rm -f "$OUT"/*

FIXTURE="$OUT/real_duplicated_short_for_minlen.fq"
...
printf 'Running Java BBNorm minlen toss case...\n'
...
printf 'Running Rust bbnorm-rs minlen toss case...\n'
...
printf 'Minlen parity passed. Java: %sms, Rust: %sms\n' "$JAVA_MS" "$RUST_MS"
