...
OUT="${1:-tmp/real_percentile_save_rare_parity}"
mkdir -p "$OUT"
rm -f "$OUT"/*
...
run_case() {
  ...
  printf 'Running Java BBNorm percentile/save-rare case %s...\n' "$name"
...
PYTIME
  ...
  printf 'Running Rust bbnorm-rs percentile/save-rare case %s...\n' "$name"
...
run_case dp20 t "percentile=20"
run_case dp80 t "percentile=80"
run_case hdp50_ldp10 t "highdepthpercentile=50" "lowdepthpercentile=10"
run_case mixed_percentiles_higher_depth f "percentile=80" "highdepthpercentile=50" "lowdepthpercentile=10"

printf 'Percentile/save-rare parity passed. Outputs and logs: %s\n' "$OUT"
